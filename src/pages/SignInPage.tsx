...
const SignInPage = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      try {
        const { data } = await axios.get<ProfileInfoResponse>('/api/question', {
          headers: { Authorization: `Bearer ${accessToken}` },
        })
        hasProfile = !!data?.hasProfile
      } catch {
        // 404/500 등 오류 시엔 프로필 없는 것으로 간주하고 질문 페이지로 유도
        hasProfile = false
      }
      ...
      if (hasProfile) {
        ...
        // 질문 페이지 경로는 사용 중인 라우팅에 맞게 변경
        alert('질문에 대한 답변을 완료해주세요.')
        navigate('/question') // 예: '/question' 또는 '/question/setup'
      }
    } catch (error: unknown) {
      if (axios.isAxiosError(error) && error.response) {
        const message = error.response.data?.message || '로그인에 실패했습니다.'
