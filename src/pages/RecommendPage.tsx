...
const pickName = (d: RawMyPage): string | null =>
  d.nickname ??
  d.name ??
  d.user?.nickname ??
  d.user?.name ??
...
const RecommendPage = () => {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const res = await API.get<Envelope<RawMyPage>>('/api/mypage')
        const raw = unwrap(res.data)
        const name = pickName(raw) ?? cachedName
